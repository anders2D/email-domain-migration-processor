...
# WAF Web ACL
resource "aws_wafv2_web_acl" "email_api_waf" {
  name  = "email-processor-waf"
  scope = "REGIONAL"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "email-processor-waf"
    sampled_requests_enabled   = true
  }
}

# Note: API Gateway v2 (HTTP API) does not support WAF association
# WAF can only be associated with API Gateway REST API or CloudFront
# The WAF rules are created and can be manually associated with CloudFront if needed
